...
Held by: no network call blocks first paint (`.docs/04-data-flow.md` §11) · synchronous MMKV rehydration (<30 ms) · Hermes bytecode precompilation · lazy route segments for map/documents/charts/Ask · no synchronous work in the root layout · fonts bundled, not fetched.
...
Specified in `.docs/07-gis-mobile-architecture.md` §9: basemap ≤1.5 s, data layer ≤2.5 s, ≥50 fps pan, ≤400 features, ≤180 MB incremental.
...
There are almost no images — a deliberate advantage. The document viewer renders page-at-a-time via `Range` requests (`.docs/10-source-traceability.md`), never a full download to render page 42.
...
### Caching
Stale-while-revalidate everywhere (`.docs/04-data-flow.md` §4): a cached screen paints in ~150 ms while refreshing behind. Persisted query cache means the *second* cold launch is fast even on a dead connection.
...
### In production (`.docs/16-observability.md`)
...
`docs/14` grows this from ~10³ admin units to ~10⁶ and from ~10⁶ fact rows to ~10¹⁰. Performance work is scheduled against those phases, not deferred indefinitely:
