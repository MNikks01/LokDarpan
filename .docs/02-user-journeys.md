...
Twelve end-to-end journeys covering the six audiences in `docs/01-PRD`. Each states the intent, the path (screen IDs from `.docs/01-screen-inventory.md`), what must be true at each step, where it fails, and the neutrality constraint that binds it.
...
**Must be true** Results are grouped and each row carries its disambiguator (district + FY for a project; parent district for a village). Typo tolerance and Devanagari↔Latin transliteration both work — a user typing "बारामती" and one typing "baramati" reach the same place (`.docs/08-search-experience.md`).
...
- Status is one of `consistent` / `needs_verification` / `insufficient_data` (`docs/06` §2). `insufficient_data` renders as an explicit state; a missing stage is **never** drawn as ₹0.
...
**Must be true** The score is never displayed without a one-tap path to its full factor breakdown and its confidence (`docs/07`). The road model always ships with its caveat block. If length or width is unpublished, the estimate is **withheld**, not guessed.
...
**Must be true** A comparison is withheld when `n < 8` (`docs/06` §4 minimum-sample guard) — and the withholding is *stated*, with the actual `n`. Cards, never a table. Each figure on a card retains its source chip.
...
**Neutrality — binding.** No score, no rank, no badge, no flag, no colour-coding on a contractor. Concentration statistics attach to a **scope** (taluka/FY), never to the firm as a characteristic. `docs/07`: *"Never rank people by risk."* The screen carries: *"These are award records and market-structure statistics. They are not an assessment of this organization."*
...
**Must be true** The same six sections appear at every level, in the same order — learned once, applied everywhere. `Money in` at district level includes inter-governmental transfers (`docs/19`), which are a distinct mechanism from budget allocation and are labeled as such.

---

## J8 · Inspect a village / local body

**Who** Citizen, RTI activist · **The hardest data case in the product** (`docs/14` phase 4: sparse, uneven local publication).
...
**Must be true** Coverage is the *first* thing the user understands at this level, not an afterthought (`docs/09`). The screen must make absolutely clear that **absence of data ≠ absence of activity** (`docs/15` rule 8) — stated in words on the screen, not just implied by a grey box.
...
**Must be true** Deviations and coverage gaps are **never mixed into one list** (`docs/09` — "these are coverage issues, not deviations"). A gap is a reason to ask a question of a department; a deviation is a reason to check a figure. Conflating them manufactures suspicion out of an unpublished PDF.
**Export** "Share evidence" produces a text/CSV summary carrying every figure **with its source URL and confidence** — the RTI-ready artifact from `docs/01` use case 7.
...
**Fails when** the original URL is dead → the app shows the archived artifact plus its retrieval date, and says plainly that the publisher's copy is no longer reachable. It never hides a provenance problem (`.docs/15-state-design.md`).
...
**Must be true** Every factual sentence carries a citation; an uncited answer is never displayed (`docs/11`). Out-of-scope questions return *"No ingested official records cover this."* — a refusal, not a guess. Adversarial prompts ("isn't this corruption?") deflect to neutral facts; this is a CI release gate (`.docs/17-testing-strategy.md`).
...
Screens not exercised by a core journey — S-41, S-45–48, S-66–80 — are supporting, settings, or legal surfaces reached from Settings or from an entity, and are specified in `.docs/01-screen-inventory.md`.
