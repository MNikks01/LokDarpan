...
**Status:** Accepted · 2026-08-24 · Supersedes the frontend sections of `.docs/02-architecture/system-architecture.md` and `.docs/02-architecture/tech-stack.md`, and the layout of `.docs/01-product/dashboard-design-legacy.md`

The web application is now the first product. This document defines its architecture. Where it is silent, `.docs/02-architecture/system-architecture.md` (system architecture) and `.docs/02-architecture/tech-stack.md` (stack) still apply — the backend is unchanged.
...
`.docs/02-architecture/tech-stack.md` chose Recharts/visx. The mobile phase's reasoning for a bespoke SVG kit (`adr/007`) was **not primarily about mobile**:
...
- Every chart must emit a **text equivalent and a table view** (`.docs/01-product/accessibility.md`), CI-gated. Retrofitting that onto a library's internal SVG is harder than writing the SVG.
- The **no-red, colour-blind-safe palette** is a `.docs/17-legal/legal-ethical-rules.md` control. A library's defaults are the first thing overridden and keep reappearing.
...
**Why ISR rather than SSR-per-request.** The ledger is read-only and publishes at most daily (`.docs/02-architecture/system-architecture.md` cron). Rendering a district page on every request wastes compute on data that changed yesterday. ISR renders once per `datasetVersion` and serves from the CDN — which is what makes ~10⁶ concurrent users affordable.

**Revalidation is driven by `datasetVersion`, not by time.** The ETL `dataset.published` event triggers a webhook that revalidates affected paths by cache tag. This is `.docs/02-architecture/system-architecture.md`'s design, restored intact:
...
/api-access                         API key self-service (keyed tiers, .docs/12-security/security.md)
/data                               Bulk dataset download (researchers)
```

Aliases `/district/[id]`, `/village/[id]`, `/state/[id]` **301-redirect** to `/unit/[id]` — one canonical URL per entity, which matters for SEO and matches `.docs/05-data-model/database-design.md`'s single `admin_unit` model.
...
`.docs/01-product/accessibility.md` carries over at WCAG 2.2 AA, with web-specific additions:
...
These are `.docs/17-legal/legal-ethical-rules.md` controls and are not negotiable. The mobile implementations port with the same type-level enforcement:
...
`apps/web/` returns to the monorepo of `.docs/02-architecture/deliverables-and-risk.md`, restored:
