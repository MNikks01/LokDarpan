...
> *"Every number must be traceable. No figure is displayed without a link to its source document, extraction method, and retrieval date."* — `docs/15`, rule 5.
...
**Architectural rule: provenance is embedded, never fetched.** Every figure in every payload carries its `provenance` object (`.docs/04-data-flow.md` §7). Consequences that matter:
...
- A figure that arrives without provenance is **not rendered** (`.docs/05-mobile-architecture.md` §2); it renders as a `MissingProvenance` placeholder and logs a contract violation. Absence of a source is a defect, not a display variation.
...
★ = a gap in the current `docs/10` contract; see `.docs/18-mobile-api-contract.md`.
...
**Security.** Only hosts present in the source registry are loadable; no script execution; no external resource loading; a 50 MB per-request cap; rendering in a sandboxed viewer. See `.docs/13-mobile-security.md`.
...
Step ④ implements `docs/15` rule 9 (preserve historical versions) — a superseded value is visible with its own source and date, so a budget revision is a fact the reader can see, not a silent overwrite.

---

## S-56 · Source registry

`docs/18`'s registry, in the app, with live operational status per source:
...
**A figure is never exported without its source columns.** An exported number that has been separated from its provenance is precisely the artifact `docs/15` exists to prevent, and it is why chart-to-PNG export is not built (`.docs/01-screen-inventory.md` §Screens that must not exist).
