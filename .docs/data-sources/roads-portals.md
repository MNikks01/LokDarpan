...
This maps onto `docs/05` `roadClass` (`NH`/`SH`/`MDR`/`ODR`/`rural`/`urban`) — **the authority differs per class, so each class needs a different source**. A single "roads" connector will not work.

## Cost-model inputs (`docs/08`)

`docs/08` needs Schedule of Rates and IRC/MoRTH specifications to compute expected cost.
...
- IRC codes — IRC is a registered society; its standards are typically **sold, not published free**. `docs/08`'s coefficient table may not be sourceable from a free official publication. **Flagged as an open licensing question.**
