...
> Sources for the `Revenue → Budget → Allocation → Release → Expenditure → Accounts → Audit` chain of `docs/13`/`docs/19`.
>
...
| Revised Estimate | **AVAILABLE** | India Budget ✅ (supports `docs/05` `estimateType` BE/RE/actual) |
...
**The gap that matters:** expenditure is published by budget head and DDO, not by project. Attributing spend to a specific work is unresolved — see [`entity-linking.md`](./entity-linking.md).

## Note on format

Almost everything here is **PDF** (grade D). `docs/03`'s PDF/table-extraction pipeline is not optional — it is the primary ingestion path for the money-in side of the ledger.
