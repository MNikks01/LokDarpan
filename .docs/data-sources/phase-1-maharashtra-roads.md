...
> The prioritised, verified source set for the first production scope (`docs/01`, `docs/14` Phase 1).
...
`docs/03` assumes a source `mh_pwd_works` supplying `work_id`, `project_name`, `allocation_amount`, `expenditure_amount`, and physical progress. **That dataset was not located.** `pwd.maharashtra.gov.in` is live, but whether it exposes a queryable works MIS with per-work progress and expenditure is unknown.
...
BEAMS is the state's budget estimation, allocation and monitoring system — on paper, exactly the `Allocation → Distribution` stage of `docs/06`. Whether it publishes allocation by department/scheme/DDO **without authentication** was not determined.
...
7. **Locate the Maharashtra Schedule of Rates** for `docs/08`, and resolve the IRC licensing question ([`resource-boq-sources.md`](./resource-boq-sources.md)).
...
**The product should be planned against this table, not against the assumption that the full chain is available.** `.docs/15-state-design.md`'s `insufficient_data` state and the coverage-first design were built for exactly this, and this pass validates that choice.
