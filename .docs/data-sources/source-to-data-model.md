...
> How a government record becomes a row in `docs/04` (§23).
...
Government Source          →  the portals in this registry
      ↓
Raw Document               →  immutable, sha256-addressed artifact (docs/03)
      ↓
...
Entity Resolution          →  admin_unit / project / contractor / scheme match
      ↓
Canonical Entity           →  docs/04 tables, with provenance + confidence + version
      ↓
Financial Ledger           →  allocation / release / expenditure
      ↓
Analytics                  →  docs/06 variance, cost/unit, peer median
      ↓
Mobile API                 →  .docs/18-mobile-api-contract.md
```
...
`docs/04` requires provenance on every fact. This registry supplies its inputs:
...
The `verification_status`, `http_status`, `final_url`, `page_title`, `verified_at` and `verification_channel` fields in [`source-registry.json`](./source-registry.json) map onto `data_source` and support the weekly link-health check `docs/02` already specifies.

**Note:** several registry entries carry a `license` of `unknown`. `docs/15` requires displaying the issuing authority and applicable licence for each dataset. **Licence terms were not captured for any source in this pass** — that is a required field before ingestion, not an optional one.

## Unmapped — the gaps

| `docs/04` element | Source status |
|---|---|
...
| `road.length_km` / `width_m` / `surface_type` | No source verified — required by `docs/08` |
...
**Five of `docs/04`'s core project-and-execution fields have no verified source.** That is the honest state after discovery, and it is the agenda for the next phase.
