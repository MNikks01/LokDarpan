...
> How each source class would have to be collected, and what that means for `docs/03`.
...
| Budget documents | **PDF extraction** (Camelot/pdfplumber) | `docs/03`'s existing design applies |
...
This maps directly onto `docs/03`'s declarative YAML source registry: one `gepnic` connector type plus per-state config, rather than 36 connectors.
...
`docs/15` requires honouring access terms; `docs/03` requires polite, scheduled collection. Nothing in this pass changes that, and two rules are absolute:
...
**No scanned PDF was confirmed in this pass.** `docs/03`'s OCR pipeline is likely still needed — state and local-body documents are where scans typically appear — but this is an expectation, not a verified finding.
...
Since `docs/01` names historical comparison as a core capability, archive depth per procurement portal is a priority question for the next phase.
