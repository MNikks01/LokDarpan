...
Current distribution across the 99 registry entries is heavily **C and D** — HTML portals and PDF documents. This matches `docs/03`'s assumption that PDF/OCR and scraping are core pipeline capabilities, and confirms that an API-first ingestion design would serve almost none of the real sources.
...
**Therefore every unreachable host in this registry is recorded as *"not reachable from the verification vantage point on 21 Aug 2026; existence not disproven"*, never as unavailable** (§35).
...
URL decay is measurable here: **7 of 36 entries in the government's own published procurement list were dead** (see [`procurement-portals.md`](./procurement-portals.md)). A registry built once and trusted will rot.

| Check | Frequency |
|---|---|
| Link health across the registry | Weekly (matches `docs/02`'s weekly link-health cron) |
