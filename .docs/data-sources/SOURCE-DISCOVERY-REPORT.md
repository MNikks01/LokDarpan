...
Predominantly **C** (HTML portals) and **D** (PDF documents). Only one confirmed general-purpose API (`data.gov.in`) and one registration-gated API (LGD NAPIX). **An API-first ingestion design would serve almost none of the real sources** — `docs/03`'s scraping-and-PDF pipeline is the correct architecture.
...
**Not established:** which fields any portal exposes; CAPTCHA; archive depth; APIs. → [`procurement-portals.md`](./procurement-portals.md)

### Administrative hierarchy — **strong**
**LGD is the single best find of this pass.** 36 States/UTs · 784 districts · 7,092 sub-districts · 7,323 blocks · **677,367 villages** · rural and urban local bodies with ward mappings · unique codes · **historical change tracking with government-order references** · 58+ reports · state/district downloads · a registration-gated API. It maps directly onto `docs/04`'s `admin_unit` including the `valid_from`/`valid_to` versioning columns, and it is already the documented integration key between MoPR systems (LGD ↔ eGramSwaraj ↔ PFMS). → [`administrative-hierarchy-sources.md`](./administrative-hierarchy-sources.md)

### Finance & budget — **medium**
Union budget, Demand for Grants, CGA monthly accounts and Finance/Appropriation Accounts are published, as PDFs. Maharashtra has a full treasury stack verified live (Mahakosh, IFMS, **BEAMS**, GRAS, ARTHWAHINI). **Expenditure is published by budget head and DDO, not by project** — the attribution gap. → [`finance-portals.md`](./finance-portals.md)

### Project execution & progress — **weak. The critical gap.**
No works register located. No verified source for work order, start date, planned completion, extensions, physical progress, financial progress, bills, completion, or per-project final cost. → [`project-monitoring-portals.md`](./project-monitoring-portals.md)

### Contractors — **medium, with a structural problem**
CPPP award records and debarment lists are public endpoints. But there is **no national contractor registry**, no confirmed link from a procurement award to a CIN or GSTIN, and 36 independent state systems each with their own vendor IDs. Cross-state contractor identity is fuzzy-only and should be out of scope for Phase 1. → [`contractor-portals.md`](./contractor-portals.md)

### GIS — **unassessed**
Bhuvan, Bharat Maps, NRSC and MRSAC all verified live. **No service endpoint (WMS/WFS/WMTS) was enumerated for any of them**, and boundary geometry is not in LGD — so the code↔geometry join is unverified. → [`gis-portals.md`](./gis-portals.md)

### Audit — **available as documents**
CAG verified live. Narrative PDFs, not structured records. State AG offices and local-body audit not located. Recommended treatment: a **document class linked to entities and cited**, never a feed into the analytics engine — an auditor's conclusion is categorically different from `docs/06`'s arithmetic. → [`audit-portals.md`](./audit-portals.md)

### Local government — **identity yes, money no**
LGD gives every local body a code. eGramSwaraj (rural accounting and progress) was unreachable; **no national urban local-body finance system was identified at all**. This matches `docs/14` Phase 4's expectation of sparse local publication, and validates the coverage-first design of the mobile Unit screen. → [`local-government-portals.md`](./local-government-portals.md) · [`panchayat-portals.md`](./panchayat-portals.md) · [`municipal-portals.md`](./municipal-portals.md)

### Resources / BOQ — **weak**
Tendered BOQ may be extractable from tender-document attachments (PDF/XLS). **Actual executed quantities, labour and equipment were not identified in any source.** A separate open question: `docs/08`'s coefficients may derive from IRC standards, which are **sold rather than freely published** — a licensing issue for the road model. → [`resource-boq-sources.md`](./resource-boq-sources.md)
...
3. **OMMAS** *(unverified)* — would close the execution gap for rural roads in one source.
4. **India Budget + CGA** — the money-in side.
5. **Maharashtra BEAMS** *(surface unverified)* — closest thing found to a state allocation system.
...
1. **Per-project expenditure** — breaks `docs/06`'s core variance.
...
7. **Licence terms** — captured for no source, though `docs/15` requires displaying them.
...
Alongside those: **check `robots.txt`, terms of use and rate limits for every source before writing a connector** (§24), and **capture licence terms**, which `docs/15` requires and this pass did not collect.
...
Throughout, §35 was applied: nothing is recorded as unpublished because it could not be found. Unreachable hosts are recorded as *"not reachable from the verification vantage point on 21 August 2026; existence not disproven."*
