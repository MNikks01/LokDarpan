...
2. **A missing source.** A figure rendered without provenance breaks `docs/15` rule 5 — the platform's central promise.
3. **Accusatory language.** A copy string, a locale file, or an AI answer that implies wrongdoing. `docs/15` makes this a release blocker, not a bug.
...
`domain/` has no React and no I/O (`.docs/05-mobile-architecture.md`), so it runs in plain Node, fast, with property-based tests where the input space is large.
...
- Locale variants (en/mr/hi) keep Latin digits (`.docs/12-accessibility.md`).
...
| `OfflineState` | O2 copy explicitly distinguishes offline from unpublished (**R2** of `.docs/15-state-design.md`) |
...
- Every Zod schema in `data/contracts/` is validated against **recorded fixtures** derived from `docs/10` + `.docs/18-mobile-api-contract.md`.
...
- **Payload budgets asserted** (`.docs/14-performance.md`): a composite fixture exceeding 60 KB gzipped fails the test.
...
1. **Core journey** — launch → search → project → Money Trail → source sheet → document page (`docs/01` use case 1, end to end).
...
This is what makes `docs/15` enforceable rather than aspirational. Each runs on every PR and blocks merge.

**G1 · Neutrality lint** (`scripts/neutrality-lint`)
Scans `i18n/*.json` (**all locales**), JSX string literals, and E2E fixtures for the forbidden vocabulary of `docs/15` — corrupt, scam, fraud, stolen, theft, embezzle, guilty, illegal, criminal, bribe, kickback, divert, siphon, loot, suspicious, and their Marathi/Hindi equivalents — plus causal constructions ("because the contractor…", "due to misuse"). Any hit fails the build. The same word list the platform's CI uses (`docs/15` §Enforcement), shared as a package so client and server cannot drift.
...
The **AI guardrail evaluation** — red-team prompts, golden-set factuality, neutrality classification — lives server-side and is a gate on the AI service (`docs/11` §Evaluation, `docs/16` M4). The client's obligation is narrower and is tested here: an answer with zero citations is dropped, a partial stream is discarded, and no AI text is rendered above the source-linked ledger.
...
Pre-release:   + manual a11y walkthrough · manual perf on reference device
               · legal/neutrality copy review (docs/15)
```

**Definition of done for a feature:** domain logic unit-tested · components tested by role/label · contract test with negative cases · one integration test · states from `.docs/15-state-design.md` implemented and tested · a11y verified at 200% · guardrails green · performance budget checked.
