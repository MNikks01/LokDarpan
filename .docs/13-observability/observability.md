...
**Sentry** (already in `.docs/02-architecture/tech-stack.md`), with a hard `beforeSend` filter:
...
Sampled at 10%. Thresholds are the ceilings in `.docs/02-architecture/performance.md`. **p75 is tracked alongside p50**, because the reference low-end device sits nearer p75 of the installed base and a p50-only dashboard hides the experience of the primary audience.
...
`.docs/17-legal/legal-ethical-rules.md` requires a CI gate on forbidden language. Production adds two runtime counters:

- **`contract_violation{kind: 'missing_provenance'}`** — a figure arrived without provenance and was suppressed (`.docs/02-architecture/data-flow.md` §12). Target zero; each occurrence is a `.docs/17-legal/legal-ethical-rules.md` rule-5 breach in the making.
