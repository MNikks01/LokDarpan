...
Internal (EAS internal distribution)
   → TestFlight / Play internal — includes the legal/neutrality copy review (docs/15)
...
**Versioning.** SemVer for the user-facing version; monotonically increasing build numbers, auto-incremented by EAS; the build number is what `/meta/client-support` compares against (`.docs/18-mobile-api-contract.md`).
...
4. **Signed and verified** (`.docs/13-mobile-security.md` §9).
5. Channels track environments: `production`, `preview`, `development`.

**Why this matters here specifically:** if an accusatory string, a mislabelled variance, or a misformatted crore figure reaches production, it is a `docs/15` compliance failure that must be corrected in hours, not days. OTA is the mechanism that makes that possible, and that is the justification for accepting the dependency.
...
**Bitrise / CircleCI / Codemagic.** Capable. Rejected: another vendor alongside GitHub Actions (already used platform-wide, `docs/12`), and EAS integrates natively with the Expo toolchain we have chosen.
...
- A copy or formatting defect can be corrected the same day — the operational counterpart to `docs/15`'s CI language gate.
- Every release carries source maps, so a production crash maps to real code.
- **Guardrails G1–G8 are merge gates, not advisory checks.** A neutrality or provenance violation cannot reach a release branch, which is what makes the enforcement claims in `docs/15` §Enforcement true rather than aspirational.
