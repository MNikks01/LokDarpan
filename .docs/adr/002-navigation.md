...
**Three tabs + header search icon.** Rejected: Search is the journalist's and RTI activist's primary path (`docs/01`); demoting it to an icon buries the workflow of two of the six named audiences.

**Ask/AI as a fifth tab.** Rejected, and this is a product decision as much as a navigation one. A standing chat surface invites unscoped questions, most of which the ledger cannot answer — producing a stream of `docs/11`-mandated refusals that teach users the feature is broken. Scoped entry (`Ask about this district`) guarantees the retriever can serve the question and keeps the answer manifestly about *these records* (`.docs/09-ai-mobile-experience.md`).
...
- **File-based routing constrains route shapes.** Acceptable; the tree in `.docs/03` maps cleanly onto it.
...
- Deep linking is nearly free and cannot drift (`.docs/21-deep-linking.md`).
