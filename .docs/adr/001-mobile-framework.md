...
- **Public-interest, grant-funded** (`docs/01`). Small team, long horizon, contributor-friendly (`docs/14` §Organizational scaling).
- **Auditable and cheap to run** (`docs/12`) — the methodology, including the client, must be inspectable.
...
- The team's existing skill base is TypeScript/React (`docs/12`: Next.js, React Query, TypeScript across the platform).
- Target device is a low-end Android phone (`.docs/14-performance.md`), not a flagship.
...
**Flutter.** Excellent rendering consistency and performance; strong map and chart ecosystems. Rejected: Dart discards the team's TypeScript skills and, more importantly, **breaks type sharing with the backend** — `docs/05`'s TypeScript models and the `api-contract` Zod schemas (`.docs/23-repository-structure.md`) are consumed directly by an RN client and would need a hand-maintained Dart port. That port is precisely where `00-document-audit` C1–C4-class defects reappear.
...
| Background change detection (`.docs/22-notifications.md`) | `expo-background-task` |
...
New Architecture + Hermes are the performance floor for the reference device: lower bridge overhead, faster startup, smaller memory (`.docs/14-performance.md`).
...
- **App size floor.** RN + Expo carries a baseline (~15–20 MB). Mitigated with per-ABI Android splits and Hermes bytecode; budget in `.docs/14-performance.md`.
...
- Type and contract sharing with the backend via `packages/api-contract` and `packages/shared-types` — the strongest available defence against the contract defects catalogued in `.docs/00-document-audit.md`.
- OTA updates allow same-day correction of a copy or formatting defect. **This matters for `docs/15` compliance:** if an accusatory string or a misformatted figure reaches production, we do not wait days for store review.
...
`docs/17` leaves the code license undecided. **AGPL-3.0 has well-known friction with the Apple App Store's distribution terms** and has caused apps to be pulled. Since this decision affects whether the product can ship at all, it must be resolved before the first submission. Recommendation: **Apache-2.0 or MPL-2.0** for `apps/mobile` — both preserve the auditability `docs/12` requires (the source is public and inspectable) without the distribution conflict. If the project wants copyleft for the backend, license the repository per-package.
