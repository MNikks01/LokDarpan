...
Maps are core (`docs/20`, `.docs/07-gis-mobile-architecture.md`). Requirements:

- Consume the **MVT tile pyramid the GIS service already builds** (`docs/20`) — this is decided infrastructure, not a choice.
...
- `docs/14` projects growth to ~10⁶ admin units and a potential ~10⁶ concurrent users.
- `docs/12`: the stack must be **cheap to run, open-source, and auditable**. `docs/01`: grant-funded, no revenue model that could compromise neutrality.
...
**`@rnmapbox/maps` (Mapbox GL Native).** The strongest technical option: most mature RN map SDK, excellent New Architecture support, first-class offline region packs, battle-tested. **Rejected primarily on cost and independence.** Mapbox prices by monthly active users; at `docs/14`'s Phase-8 scale that is a recurring bill a grant-funded public-interest platform cannot underwrite, and a dependency whose pricing could change would sit on the product's core surface. Since we self-host the tiles anyway (`docs/20`), Mapbox's real value-add — its basemap and its data — is not what we would be paying for. There is also a fit argument: a platform whose credibility rests on auditability should not have a proprietary black box rendering its primary public surface.
...
- We operate the tile infrastructure — but `docs/20` already commits to generating and serving the MVT pyramid, so this is not new work.
- Basemap cartographic quality is below Google/Mapbox. Acceptable and arguably desirable: a deliberately muted basemap keeps all colour for the data layers (`.docs/06-design-system.md`).
...
- Fully open-source rendering path, consistent with `docs/12`'s auditability commitment.
- Offline packs are a first-class capability (`.docs/11-offline-strategy.md`).
- Server-side clustering and precomputed metrics mean the client never computes a map value — so a map and a table can never disagree (`docs/20`).
