...
**Legend** — ★ new backend requirement (`.docs/18-mobile-api-contract.md`) · Cache: `Q`=TanStack Query→MMKV, `D`=SQLite durable, `H`=HTTP/CDN, `L`=local-only, `—`=none · Auth: all `public` (anonymous) unless stated · Offline: `Full`=works from cache/durable · `Partial`=degraded, stated · `Cached`=cache only · `No`=explicitly unavailable, with copy.
...
| S-74 | Legal & neutrality | bundled (`docs/15` verbatim) | — | — | — | — | Full |
