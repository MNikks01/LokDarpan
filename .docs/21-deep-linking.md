...
**Why aliases exist and why they redirect.** The brief lists `lokdarpan://district/{id}` and `lokdarpan://village/{id}`. These are accepted for readability and for links already in circulation, but they resolve to the single canonical `/unit/{id}` route — because `docs/04`/`docs/19` model every level as one `admin_unit`, and two route families for "a place" would recreate the `district`/`admin_unit` duality that `00-document-audit` C7 identifies as a maintainability trap.

**LGD codes as an alternative key** ★: `/unit/lgd/{lgdCode}` is a requested addition. LGD codes are the government's own cross-source identifier (`docs/19`) and are stable across our internal id changes — which makes them the right thing to print in an article or an RTI application.
...
Rules (`.docs/13-mobile-security.md` §6):
...
7. **Fuzz-tested** in CI with malformed, oversized, injected, and unicode-confusable links (`.docs/17-testing-strategy.md`).
...
Constraints, from `docs/15`:
- The share text carries **figures and the link**, never an observation, a verification-priority score, or an AI-generated sentence. A score pasted into a group chat without its factor breakdown is exactly the decontextualised judgment `docs/07` exists to prevent.
- The line *"Figures from official records"* is mandatory in every share payload.
- No screenshot generation, no chart-to-image export (`.docs/01-screen-inventory.md` §Screens that must not exist) — an image separates a number from its provenance.
...
Local notifications (`.docs/22-notifications.md`) carry a deep link to the changed entity, **anchored to the changed section** — `lokdarpan://project/501/finance?highlight=expenditure`. The `highlight` parameter scrolls and outlines a section; it never injects content.
