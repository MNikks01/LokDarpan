...
`docs/13` sets the platform posture: the data is public, so **integrity, availability and traceability** are the priorities, not confidentiality. The mobile client inherits that and adds its own surfaces. This document covers only what is client-side or client-triggered.
...
| AI questions | Same. Server-side `docs/11` audit logs carry no user identifier, so they cannot be joined to a person |
| Watchlist / saved items | **On-device.** Change detection is a client-side `?since=` poll, so the server never learns which projects a person monitors (`.docs/22-notifications.md`) |
...
| Screenshots of financial figures | Not blocked (users need to share evidence), but shared artifacts always carry their sources (`.docs/10-source-traceability.md`) |
...
- **No action links.** No deep link can save, delete, change a setting, sign in, grant a permission, download, or submit a report. Links are strictly navigational (`.docs/21-deep-linking.md`).
...
- SBOM generated per build and published — consistent with the platform's auditability commitment (`docs/12`).
...
- **One `datasetVersion` per screen.** Sections from different versions are never rendered together (`.docs/11-offline-strategy.md` §Conflict).
...
- **No provenance ⇒ no render** (`.docs/05-mobile-architecture.md` §2). Structurally, an unsourced figure cannot reach the screen.
...
| Data-tamper suspicion upstream | `docs/13` platform runbook; the client shows the affected `datasetVersion` as "under review" and offers the prior version |
...
- [ ] Independent security review before public launch, and before each expansion phase (mirroring `docs/15`'s legal-review cadence)
