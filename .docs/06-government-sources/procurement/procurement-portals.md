...
> Every official government procurement system identified, Central and State/UT. This is the single most important source category for LokDarpan (`.docs/00-overview/platform-overview.md` tender→contractor→release chain).
>
> **Verification date:** 21 August 2026 · **Status model:** `DISCOVERED` → `VERIFIED` → `PRODUCTION_READY` (see [`.docs/06-government-sources/source-quality.md`](../source-quality.md)).
...
This is a significant engineering result: **one connector can serve most States/UTs**, with per-state exceptions. See [`.docs/04-data-engineering/ingestion-methods.md`](../../04-data-engineering/ingestion-methods.md).
