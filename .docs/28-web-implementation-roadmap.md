...
**Status:** Accepted · 2026-08-24 · Supersedes `.docs/25-implementation-roadmap.md` as the active plan (25 is retained for the deferred mobile build)

Replaces Month 3 of `docs/16-Development-Roadmap.md` with a fuller plan; Months 1, 2, 4, 5, 6 of `docs/16` (backend, analytics, AI, anomaly/risk, hardening) stand, with the contract additions in `.docs/18-mobile-api-contract.md`.
...
Web is blocked on fewer items than mobile was. The BFF is no longer P0 (`.docs/26` §Backend requirements).
...
**Exit:** `<Figure>` without `provenance` fails to compile; `<Observation>` with a literal fails to compile; the neutrality lint blocks a PR; no red token in any band. **`docs/15` is structurally enforced before any page exists to violate it.**
...
Accessibility sweep (keyboard, screen reader, 200% zoom, 320px) · Core Web Vitals pass · security review · **legal/neutrality copy review against `docs/15` in all three locales** · load test · monitoring · launch.
**Exit:** every quality-gate item green; external accessibility and security review.

**Ask (AI)** is deliberately not in the launch path — it is advisory and secondary (`docs/11`: if AI and ledger disagree, the ledger wins), and depends on the most backend work. Add after launch.
...
| **Execution-data gap** (`.docs/data-sources/`) | **High** | Per-project expenditure and progress have no verified source. Plan the Money Trail to render `insufficient_data` honestly and lead with tender/award and budget data, which *are* verified |
...
1. Ask (AI) surface
2. **Mobile app** — resume `.docs/00`–`25` and `adr/001`–`010`, revalidating the toolchain choices against the then-current Expo/RN releases
3. Saved items and notifications, with the on-device privacy design from `.docs/22`
4. `docs/14` domain and geographic expansion (Phase 2 onward)
