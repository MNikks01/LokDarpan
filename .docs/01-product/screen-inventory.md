...
Every screen the mobile product requires, with its purpose, data, states, and instrumentation. Screens carry stable IDs (`S-nn`) used by `.docs/11-api/screen-api-matrix.md`, `.docs/05-data-model/screen-data-matrix.md`, and the wireframes.
...
- Panel 1: _what LokDarpan does_ — follows official money, links every number to its government source.
  Panel 2: **what it does not do** — verbatim from `.docs/17-legal/legal-ethical-rules.md` §Positioning: _"A difference or gap shown here means the data warrants a closer look — it is not a claim of wrongdoing by any person or organization."_
  Panel 3: _how to verify_ — a live demo of tapping a figure → source sheet → document page.
...
- **Purpose** Explain _why_ location is wanted before the OS dialog, and offer a first-class manual alternative. The OS prompt is never fired blind.
- Copy states plainly: coordinates are used to find nearby records and are never stored or linked to you (`.docs/12-security/mobile-security.md`).
...
- **Purpose** Answer _"what public money is around me, and what should I look at?"_ — an intent launcher, **not** a KPI dashboard.
...
### S-12 · Notification/update detail — _not a screen._ Notifications route directly to the entity, anchored to the changed section. Documented here so it is not built as a screen.
...
- **Events** `search_performed{result_count_bucket, types_returned, had_filters}` — **query text is never sent** (`.docs/13-observability/observability.md`)

### S-15 · Search — no results (`screen` state)

- **Purpose** Explain _why_ nothing matched and give a next action. Three distinct cases, three distinct messages:
  (a) _out of coverage_ — "LokDarpan currently covers Maharashtra roads. <term> may be outside that." + link to coverage.
  (b) _spelling_ — "Did you mean …" from the suggest endpoint.
  (c) _published but not ingested_ — "No official record matching <term> has been ingested." + link to Source Registry + Report a data issue.
...
- **Purpose** Geographic discovery. Two co-equal presentations of the same query: **Map** and **List**. The list is not a fallback — it is the accessibility- and low-end-device-equivalent surface (`.docs/01-product/accessibility.md`).
...
- Layer (boundaries / roads / projects / facilities) · metric for choropleth (utilization %, per-capita expenditure, project count, median cost/km) · category · status · FY · verification-priority band. Includes the **legend**, which states that colour encodes a measurement, not a judgment (`.docs/17-legal/legal-ethical-rules.md`).
...
> **S-23 replaces six separate dashboards from `.docs/01-product/dashboard-design-legacy.md`** (National, State, District, Village/Local-Body, plus Infrastructure and Audit scoped views). One screen, one learned layout, at every level.
...
- **Sections** (fixed order — the four-panel pattern from `.docs/01-product/dashboard-design-legacy.md` §Level-agnostic):
  ...
  3. **Money out** — releases + expenditure; the Money Trail component (`.docs/01-product/design-system.md`).
  4. **What was built** — project/asset counts by domain and status; "View on map"; "See all projects".
  5. **Consistency** — vertical roll-up check (`.docs/07-analytics/analytics-engine.md` §10), peer comparison vs. siblings (`.docs/07-analytics/analytics-engine.md` §9), observation count. Framed as _worth verifying_.
  6. **Coverage** — what is missing and which source would carry it. Prominent at local-body level (`.docs/01-product/dashboard-design-legacy.md`).
...
- **Purpose** Show the parent figure, the sum of children, the gap, the gap %, which children are missing from the sum, and the exact neutral observation. Explicitly states: _records may be incomplete_ — never diversion.
- **API** `GET /units/:id/consistency?fy=` · **Events** `rollup_viewed{gap_pct_bucket}`

### S-26 · Peer comparison (`screen`)

- **Purpose** This unit vs. its siblings on a normalized metric (per-capita, utilization ratio, per-project). Distribution strip with this unit marked, `n`, median, and the minimum-sample guard from `.docs/07-analytics/analytics-engine.md` §4.
...
- **States** L: header from route params + skeletons · E: per-section, naming the missing record type · Err: per-section retry · Off: cached; if saved, guaranteed complete offline (see `.docs/10-mobile/offline-strategy.md`)
...
### S-30a · Value history (`screen`) — every retained `record_version` for this line (budget revisions), with the source and date of each. Implements `.docs/17-legal/legal-ethical-rules.md` rule 9 (preserve historical versions) on mobile.
...
- Cost per km (actual) vs. modeled expected vs. district median, with `n`, the model version, the coefficient table, and the **mandatory caveat block** from `.docs/03-domain/road-infrastructure-intelligence.md` (_"Modeled estimates are engineering approximations with stated assumptions; deviations can be legitimate."_).
...
- The 0–100 score, its band, its **confidence**, and all six factors from `.docs/08-risk/risk-scoring-engine.md` with weight, sub-score, contribution, and a neutral per-factor note. Standing disclaimer. Never rendered without the breakdown reachable in one tap.
...
- **Descriptive statistics only** (`.docs/11-api/api-documentation.md`): canonical name, class/grade, **aliases** (transparency of canonicalization), tender count, total awarded, average bidders, scope share. A standing note explains that names were canonicalized from listed variants and that share is a market-structure statistic.
- **Explicitly absent:** any score, rank, badge, flag, or "risk" attached to the contractor (`.docs/08-risk/risk-scoring-engine.md` — never rank people).
- **Events** `contractor_viewed`

### S-43 · Contractor tenders (`screen`) — that contractor's award records, cursor-paged, each source-linked.

### S-44 · Concentration context (`screen`) — HHI and top-k share for a scope, with the standard descriptive labels from `.docs/07-analytics/analytics-engine.md` §8 and an explanation of what HHI is. Attached to a **scope**, never to a contractor.
...
- Standing disclaimer at the top, not collapsible: _"These are data-consistency observations from official records, not findings of wrongdoing."_
...
- **Purpose** Missing-record and low-confidence items, kept **separate from deviations** (`.docs/01-product/dashboard-design-legacy.md` — these are coverage issues, not inconsistencies). Lists what was expected, what is present, what is absent, and the source that would carry it.
...
- **Purpose** Reachable from _every single figure in the app_ in one tap. Shows: the value, the issuing authority, the document title, the page/table locator, the extraction method, the extraction confidence, retrieved-at and published-at, and the actions **View document** / **Open original** / **View lineage**.
...
- **States** L: from embedded provenance, instant · E: **impossible by design** — a figure with no provenance cannot be rendered (`.docs/01-product/design-system.md` §Figure) · Off: full metadata cached with the figure; document body may be unavailable, stated plainly
...
- **Security** host allow-list from the source registry, no script execution, size cap — see `.docs/12-security/mobile-security.md`

### S-55 · Lineage (`screen`) — the provenance chain for a figure: source document → extraction method + confidence → normalization applied → record version → any derived calculation that used it. Implements `.docs/01-product/source-traceability.md`.

### S-56 · Source registry (`screen`) — browse the approved official sources from `.docs/06-government-sources/legacy-source-directory.md`, with per-source status: last successful fetch, link health, records ingested, license. Makes the platform's own data supply auditable by the public.

### S-57 · Methodology explainer (`sheet`) — per-metric: what this number is, the exact formula from `.docs/07-analytics/analytics-engine.md`/`07`/`08`, its assumptions, and its limits. Attached to every derived figure via a "?" affordance.
...
- **States** L: **streamed tokens**, with the retrieval step named ("Reading 41 official records for Pune…") · E: _"No ingested official records cover this."_ — the mandated refusal from `.docs/09-ai/ai-layer.md`, not an apology · Err: falls back to the deterministic template summary from `.docs/09-ai/ai-layer.md` §Templated fallback · Off: **Ask is disabled offline**, stated plainly, with cached previous answers readable
...
### S-65 · Watch settings (`sheet`) — per saved item: which changes to be told about (new expenditure, new release, status change, new observation, source superseded). Default: figures only. See `.docs/10-mobile/notifications.md`.
...
### S-74 · Legal & neutrality (`screen`) — the positioning statement and the mandatory rules from `.docs/17-legal/legal-ethical-rules.md`, verbatim and non-collapsible.
...
### S-77 · Coverage & limitations (`screen`) — what is and is not covered today, per `.docs/01-product/prd.md` §Limitations. Reachable from every empty state.

### S-78 · Report a data issue (`screen`) — the `.docs/17-legal/legal-ethical-rules.md` right-of-reply channel: entity, figure, what's wrong, optional contact. Works offline (queued).
...
Not screens. Full behaviour in `.docs/01-product/state-design.md`; listed here so no screen is built without them.
...
Each of these is a desktop or engagement pattern that would either misrepresent the data or violate `.docs/17-legal/legal-ethical-rules.md`. They are listed so that "we forgot it" is never confused with "we decided against it."
...
| **Six separate level dashboards** (`.docs/01-product/dashboard-design-legacy.md`) | Six layouts to learn; identical information architecture.                                                     | S-23, one level-agnostic Unit screen                             |
...
| **"Worst districts / worst contractors" ranking**                                 | `.docs/08-risk/risk-scoring-engine.md` explicitly forbids ranking people or presenting risk as a leaderboard. | S-26 peer distribution (no rank), S-44 scope-level concentration |
...
| **Chart-to-PNG export**                                                           | Exports a number without its source, breaking `.docs/17-legal/legal-ethical-rules.md` rule 5.                 | Share a universal link (which carries the source)                |
