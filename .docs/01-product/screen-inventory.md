...
- Panel 1: *what LokDarpan does* — follows official money, links every number to its government source.
  Panel 2: **what it does not do** — verbatim from `.docs/17-legal/legal-ethical-rules.md` §Positioning: *"A difference or gap shown here means the data warrants a closer look — it is not a claim of wrongdoing by any person or organization."*
...
- **Sections** (fixed order — the four-panel pattern from `.docs/01-product/dashboard-design-legacy.md` §Level-agnostic):
  ...
  5. **Consistency** — vertical roll-up check (`.docs/07-analytics/analytics-engine.md` §10), peer comparison vs. siblings (`.docs/07-analytics/analytics-engine.md` §9), observation count. Framed as *worth verifying*.
...
- Cost per km (actual) vs. modeled expected vs. district median, with `n`, the model version, the coefficient table, and the **mandatory caveat block** from `.docs/03-domain/road-infrastructure-intelligence.md` (*"Modeled estimates are engineering approximations with stated assumptions; deviations can be legitimate."*).
...
- **States** L: **streamed tokens**, with the retrieval step named ("Reading 41 official records for Pune…") · E: *"No ingested official records cover this."* — the mandated refusal from `.docs/09-ai/ai-layer.md`, not an apology · Err: falls back to the deterministic template summary from `.docs/09-ai/ai-layer.md` §Templated fallback · Off: **Ask is disabled offline**, stated plainly, with cached previous answers readable
