...
| **Execution-data gap** (`.docs/06-government-sources/`) | **High** | Per-project expenditure and progress have no verified source. Plan the Money Trail to render `insufficient_data` honestly and lead with tender/award and budget data, which *are* verified |
