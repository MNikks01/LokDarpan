...
4. **Alias matching on contractors** — `.docs/04-data-engineering/data-collection-architecture.md` canonicalizes messy contractor names; search must match the *aliases*, and the result must show which alias matched, so the canonicalization stays auditable (`.docs/01-product/user-journeys.md` J6).
