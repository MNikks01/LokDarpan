...
Target: **WCAG 2.2 AA**, applied to a native app (`.docs/01-product/dashboard-design-legacy.md` commits to WCAG 2.1 AA; this raises it to 2.2, whose additions — target size, focus appearance, dragging alternatives — are all mobile-relevant).
...
- **Colour is never the only carrier** (`.docs/01-product/dashboard-design-legacy.md`): every band, severity, status and confidence pairs colour with an **icon and a text label**.
- The data palettes are colour-vision-deficiency safe by construction — a single-hue sequential ramp for magnitude and a blue↔amber diverging ramp; **no red/green anywhere** (`.docs/01-product/design-system.md`). This is simultaneously an accessibility decision and a neutrality decision.
...
- **One learned layout everywhere.** The same six sections in the same order on every Unit screen at every level (`.docs/01-product/screen-inventory.md` S-23). Learn it once at district level, apply it in a village.
