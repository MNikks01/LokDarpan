...
- **3 tabs + header search** — Search is a first-class investigative surface here (`.docs/08-search-experience.md`); demoting it to an icon buries the journalist's primary path.
...
**Stack depth guard.** When a stack exceeds **12 entries**, opening a further entity *replaces* the top entry rather than pushing, and the ancestor menu remains the way back. This bounds memory (`.docs/14-performance.md`) and prevents the 40-tap unwind.

---

## Sheets: the mobile answer to hover, popover, and drawer

`docs/09` specifies a `ProvenanceDrawer`, hover tooltips, and popovers — all desktop affordances. On mobile these become **bottom sheets**, which are reachable one-handed, dismissible by gesture, and preserve the underlying context.
...
Full scheme, universal-link setup, validation, and security: `.docs/21-deep-linking.md`.
...
- Push/pop: platform default (iOS slide, Android fade-through). No custom hero animations in Phase 1 — they cost frames on the reference low-end device (`.docs/14-performance.md`).
- Tab switch: no animation; instant.
- Sheets: spring, 300 ms, with `reduceMotion` collapsing to a 100 ms fade (`.docs/12-accessibility.md`).
