...
1. **The app always opens.** No network call blocks first paint (`.docs/04-data-flow.md` §11).
...
Conflating them turns a dropped connection into an implied accusation of non-publication against a government body. That is a `docs/15` violation dressed as a UI shortcut.
...
| Ask | **Disabled**, stated plainly; history readable (`.docs/09-ai-mobile-experience.md`) |
...
| Watchlist diff | Background fetch, ≥ 6 h apart | Wi-Fi preferred; a single `?since=` call (`.docs/22-notifications.md`) |
...
There is no write conflict to resolve — the ledger is read-only (`docs/10`) and the only local writes are saves, settings, and queued reports. What must be handled instead is **version skew**:

- A screen must never mix figures from two `datasetVersion`s. Composite endpoints guarantee one version per payload (`.docs/04-data-flow.md` §6); when two independently-fetched sections disagree, the older section is re-fetched rather than displayed alongside the newer.
- A saved bundle keeps its version. When a refresh finds a newer one, the app shows what changed (`.docs/22-notifications.md`) rather than silently replacing the numbers a journalist may have already cited.
- A superseded record retains its predecessor (`docs/15` rule 9) — offline included.
...
Offline is a first-class test surface (`.docs/17-testing-strategy.md`): airplane-mode E2E flows for launch/browse/save/source-sheet, a throttled-network suite (3G profile), a "cache present, server 500" suite, a "cache present, dataset version bumped" suite, and a storage-eviction suite. A release that has not passed the offline suite does not ship.
