...
export function GET(request: Request): Promise<Response> {
  return respond(request, async () => {
    ...
    return inLedger(async ({ facts }) => {
      if (params.get("unresolved") === "true") {
        return { documents: await facts.listUnattributedDocuments() };
      }
      ...
      return {
        documents:
          unitId === null ? await facts.listDocuments() : await facts.listDocumentsForUnit(unitId),
      };
    });
  });
}
