/**
 * The GePNIC deployments LokDarpan may collect from, and links to.
 *
 * In `packages/domain` rather than the ingestion service because the explorer
 * needs the same table to link a reader to a state's portal, and the web app
 * may not import from a collector.
 *
...
/**
 * The portal serving a state, by the state's LGD code.
 *
 * Also what the explorer links to while tender details are withheld: each
 * portal's terms permit linking to it without permission (source-licences.md §5).
 */
export function portalForState(stateLgdCode: string): Portal | undefined {
  return PORTALS.find((p) => p.stateLgdCode === stateLgdCode);
}

/** The public landing page a reader can open to browse a portal's tenders. */
export function portalHomeUrl(portal: Portal): string {
  return `${portal.baseUrl}/nicgep/app`;
}
