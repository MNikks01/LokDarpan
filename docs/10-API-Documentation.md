...
**Base URL:** `https://api.lokdarpan.org/api` (illustrative). **Version:** `v1` (prefix `/api/v1`). **Format:** JSON. **Auth:** public read endpoints are open but rate-limited; higher quotas require an API key ([13](./13-Security.md)). **All responses are read-only** — the public API never mutates the ledger.
...
| `POST /ai/ask` | Guardrailed Q&A (see [11](./11-AI-Layer.md)); body `{ question, scope }`, returns answer + citations |
...
Read-only, same envelope/provenance rules. `unitId` is an `admin_unit` id ([19](./19-Administrative-Hierarchy.md)).
...
| `GET /units/:id/consistency` | Vertical roll-up gap check ([06 §10](./06-Analytics-Engine.md)) |
...
| `GET /geo/units/:id` | GeoJSON boundary; `GET /tiles/{layer}/{z}/{x}/{y}.mvt` vector tiles ([20](./20-GIS-Intelligence.md)) |
...
- **Public reads:** open, rate-limited, no key. Higher quotas/exports need an API key (`Authorization: Bearer <key>`); roles per [13](./13-Security.md) (`public`/`journalist`/`researcher`/`analyst`/`admin`).
...
**Guardrails:** GraphQL is depth- and complexity-limited (cost analysis) to prevent abusive nested queries; persisted queries recommended for the public app; the AI `ask` field is bound by the same neutrality stack as REST ([11](./11-AI-Layer.md)).
