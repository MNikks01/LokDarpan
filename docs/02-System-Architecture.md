...
5. **Neutral by construction.** The reporting/AI layers can only emit vetted, factual statements (see [11](./11-AI-Layer.md), [15](./15-Legal-Ethical-Rules.md)).
...
Services are independently deployable containers ([12](./12-Tech-Stack.md)). The **only** write path to the canonical ledger is via ETL workers; everything downstream is read-only against it.
...
- **Design tokens** shared with the design system ([09](./09-Dashboard-Design.md)).
...
- **Rate limiting** and **RBAC** per [13](./13-Security.md).
...
Full detail in [03 — Data Collection Architecture](./03-Data-Collection-Architecture.md). Summary of stages:
...
- All admin/data mutations recorded in an **audit log** ([13](./13-Security.md)).
...
Container orchestration via Kubernetes; see [12 — Tech Stack](./12-Tech-Stack.md) and [17 — Folder Structure](./17-Deliverables-Folder-Structure.md).

## Microservices & event-driven design (national scale)

The service boundaries above are deployed as **independent microservices** (`apps/api`, `apps/web`, `apps/admin`; `services/ingestion|analytics|ai|audit|gis`; `workers/*` — see [17](./17-Deliverables-Folder-Structure.md)), each independently deployable and scalable. They coordinate through an **event-driven pipeline** rather than synchronous calls on the write path:
...
This is the same architecture at every phase; only the number of services replicas, partitions, and the event-bus backend scale up ([14](./14-Scalability-Plan.md)).
