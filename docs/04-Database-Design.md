...
- **Provenance is mandatory** on every fact row. This enforces the traceability rules in [15](./15-Legal-Ethical-Rules.md).
...
Derived quantities (variance, deviation %, cost/km) are computed by the analytics service and stored/served alongside these rollups — formulas in [06](./06-Analytics-Engine.md) and [08](./08-Road-Infrastructure-Intelligence.md).

---

# National-scale extensions (village → nation)

These tables generalize the Phase-1 schema to the full administrative hierarchy, urban/rural local bodies, schemes, and social/utility infrastructure ([19](./19-Administrative-Hierarchy.md), [20](./20-GIS-Intelligence.md)). The existing revenue/allocation/release/expenditure/anomaly tables carry over unchanged; they simply reference `admin_unit` and `scheme` in addition to `department`/`project`.
...
Same pattern as `road`/`bridge` ([Phase-1 tables above]) — each links to a `project`, an `admin_unit`, carries geometry + provenance, and adds capacity attributes that power cost-per-unit metrics ([06](./06-Analytics-Engine.md)).
...
Large fact tables (`allocation`, `release`, `expenditure`, `intergov_transfer`, `anomaly`) are **range-partitioned by `fiscal_year_id`** and, when India-wide, **list-sub-partitioned by `state_code`**, keeping per-state working sets small ([14](./14-Scalability-Plan.md)). Asset tables partition by `state_code`. Closure and `admin_unit` stay global but indexed.
