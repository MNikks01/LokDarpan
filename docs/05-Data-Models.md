...
Shared types used by the API gateway and frontend. They mirror [04 — Database Design](./04-Database-Design.md). **Money is represented as an integer/`number` of rupees plus a formatted display string**; never as a float subject to rounding surprises in the UI. Every fact type carries a `provenance` object so the traceability rule is a compile-time expectation, not a convention.
...
export type TaxHead =
  | "income_tax" | "corporation_tax" | "stamp_duty" | "vehicle_tax" | "other";
...
export type ProjectCategory =
  | "state_highway" | "national_highway" | "bridge"
  | "rural_road" | "urban_road" | "other";

export type ProjectStatus =
  | "sanctioned" | "tendered" | "in_progress"
  | "completed" | "stalled" | "unknown";
...
export type AnomalyType =
  | "variance_gap" | "utilization_exceeds_release" | "release_exceeds_allocation"
  | "cost_per_km_outlier" | "missing_records" | "budget_revision_spike"
