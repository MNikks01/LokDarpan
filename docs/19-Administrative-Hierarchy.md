...
The platform models the **complete Indian administrative and fiscal hierarchy**, so any rupee can be followed from the Union Budget down to a ward-level project and back up. This document defines that hierarchy, the urban and rural local-body structures, the geographic hierarchy, the official code systems that make cross-source linkage possible, and how money flows through the levels. The database realization is in [04 — Database Design](./04-Database-Design.md).
...
Parallel to governance, every unit and asset carries geography (detail in [20 — GIS Intelligence](./20-GIS-Intelligence.md)):
...
Where a source lacks a standard code, the ingestion layer attempts a mapping (deterministic + fuzzy) and records a **confidence**; unresolved cases are flagged, never guessed ([03](./03-Data-Collection-Architecture.md)).
...
The variance/consistency engine ([06](./06-Analytics-Engine.md)) runs **at every level**, not just per project:
