...
- Repo/monorepo scaffold, CI, containerization, environments ([17](./17-Deliverables-Folder-Structure.md)).
- PostgreSQL + PostGIS schema from [04](./04-Database-Design.md); migrations; seed dimensions (fiscal years, MH districts with geometry).
...
- Analytics service ([06](./06-Analytics-Engine.md)): variance, budget consistency, cost/km, district/historical comparison, inflation adjustment, contractor concentration.
- Materialized views (`mv_project_finance`) + nightly recompute cron.
- API gateway ([10](./10-API-Documentation.md)): `/revenue`, `/projects`, `/projects/:id`, `/projects/:id/finance`, `/districts/:id`, `/contractors/:id`, `/sources/:docId`; pagination/filtering/sorting; Redis caching; OpenAPI spec.
...
- Guardrail stack ([11](./11-AI-Layer.md)): grounding, numeric fidelity, neutrality classifier, citation enforcement, templated fallback.
...
- Risk scoring ([07](./07-Risk-Scoring-Engine.md)) with factor breakdown; "Verification Priority" UI.
- Audit view ([09](./09-Dashboard-Design.md)) with inconsistencies, warnings, and per-item explanations + evidence links.
...
- Security pass ([13](./13-Security.md)): audit logs, secrets, mTLS, backups + restore drills, WAF/rate-limit tuning, dependency/image scans.
- Provenance/versioning UX polish; correction ("report a data issue") flow.
- Legal & neutrality review against [15](./15-Legal-Ethical-Rules.md); disclaimers in place.
