...
- Every mutating action (ingest, load, version bump, entity merge, quarantine decision, export, login, config change) writes to `audit_log` with `actor`, `action`, `entity`, `detail`, `at` ([04](./04-Database-Design.md)).
...
- CI runs SAST + secret scanning + the AI guardrail eval suite ([11](./11-AI-Layer.md)) as a release gate.
