...
>
> **Sourcing rule ([15](./15-Legal-Ethical-Rules.md)):** only official government portals. No news, blogs, social media, or third-party sites are ever used as sources of fact.
...
- 🔎 **Verify** — department/treasury/e-tender subdomains change over time and differ by state. The URL shown is the expected/known location; **confirm it live before wiring a connector**, and record the confirmed URL + license in `sources/*.yaml` ([17](./17-Deliverables-Folder-Structure.md)).

This is a discovery directory, not a guarantee of live endpoints. The ingestion layer treats every URL as unverified until a connector run confirms it ([03](./03-Data-Collection-Architecture.md)).
...
**Phase 4+ (Other states):** replicate the per-state block, starting with the largest road programs (UP, MP, Rajasthan, Karnataka, Tamil Nadu), reusing the same connector templates ([14](./14-Scalability-Plan.md)).
...
# Datasets to build (canonical tables → see [04](./04-Database-Design.md))
