...
Scaling has two axes: **breadth** (more domains, more levels of the hierarchy, more states) and **throughput** (more data, more users). The architecture ([02](./02-System-Architecture.md)) is source-driven, hierarchy-generic ([19](./19-Administrative-Hierarchy.md)), and horizontally scalable, so breadth expansion is mostly *configuration + connectors + domain modules*, not rewrites.
...
Each phase reuses the same pipeline; what changes is the **source registry** ([18](./18-Data-Source-Registry.md)), the **domain taxonomy**, per-domain **intelligence modules** (roads = [08](./08-Road-Infrastructure-Intelligence.md) is the template), and the **hierarchy depth** switched on.
...
### Data volume
- **Partitioning:** fact tables range-partitioned by `fiscal_year_id`, list-sub-partitioned by `state_code` at national scale; asset tables partitioned by `state_code` ([04](./04-Database-Design.md)). Keeps per-state working sets small.
...
- **Vector tiles** pre-generated per zoom/version for national maps ([20](./20-GIS-Intelligence.md)).
